...
async function test() {
  ...
  // ========== 测试2: 取消分配不影响作者 ==========
  console.log('\n【测试2: 取消分配不影响作者】');
  // 当前是pending_assignment状态，编辑打开分配窗口但取消
  // 检查作者侧状态
  ...
  // 检查decisions中新增了reviewers_assigned记录
  ...
  const test5b = pool5.validCount === 1 && pool5.remainingNeeded === 1 && 
                 pool5.pool[2].pool_status === 'invited';  // 第3位被自动邀请补足
  ...
  const test7 = authorPaper.decisions?.length >= 3;  // screening_passed, reviewers_assigned, 可能还有其他
